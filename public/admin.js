...
function renderResults(matches, leaderboard) {
  ...
  byId('admin-matches').replaceChildren(...matches.matches.map((match)=>{const row=document.createElement('article');const data=document.createElement('div');const title=document.createElement('strong');title.textContent=match.report.map||match.report.reportId||`Partida #${match.id}`;const meta=document.createElement('span');const scope=[match.stageId?`Fase ${match.stageId}`:null,match.groupId?`Grupo ${match.groupId}`:null,match.hostId?`Host ${match.hostId}`:null,match.matchNumber?`Partida ${match.matchNumber}`:null,match.report.reportId?`reportId ${match.report.reportId}`:null].filter(Boolean).join(' · ');meta.textContent=`${new Date(match.receivedAt).toLocaleString('es-ES')} · ${match.origin||'REPORTER'} · ${match.matchStatus||'VALID'}${scope?` · ${scope}`:''}${match.voidReason?` · ${match.voidReason}`:''}`;data.append(title,meta);const actions=document.createElement('div');const voidButton=document.createElement('button');voidButton.type='button';voidButton.textContent='ANULAR';voidButton.hidden=match.matchStatus==='VOID';voidButton.addEventListener('click',async()=>{const reason=prompt('Motivo de anulación');if(!reason)return;if(!confirm(`¿Anular la partida #${match.id}? Permanecerá en el historial sin puntuar.`))return;try{await api(`/api/admin/events/${selectedEvent.id}/matches/${match.id}/void`,{method:'PATCH',body:JSON.stringify({reason})});await loadResults();feedback('Partida anulada y clasificación recalculada.');}catch(error){feedback(error.message,true);}});const remove=document.createElement('button');remove.type='button';remove.textContent='ELIMINAR';remove.addEventListener('click',async()=>{if(!confirm('¿Eliminar definitivamente este resultado? La clasificación se recalculará.'))return;try{await api(`/api/admin/events/${selectedEvent.id}/matches/${match.id}`,{method:'DELETE'});await loadResults();feedback('Resultado eliminado.');}catch(error){feedback(error.message,true);}});actions.append(voidButton,remove);row.append(data,actions);return row;}));
}
