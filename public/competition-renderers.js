(function exposeCompetitionRenderers(root, factory) {
  ...
  function championName(state) {
    ...
    return (
      playoffs.placements?.find((fila) => fila.teamId === playoffs.champion)?.name ||
  ...
  function seriesCard(context, series, { compact = false } = {}) {
      ...
      for (const game of series.games) {
        ...
        item.append(
          ...
          node(
            'strong',
            '',
            game.status === 'COMPLETED' ? `${game.teamARounds} — ${game.teamBRounds}` : statusLabel(game.status)
          )
        );
    ...
    const detail = link(
      `${competitionRoot(context.slug)}/partidos/${series.id}`,
      'series-detail-link',
      'Abrir detalle →'
    );
    card.append(detail);
  ...
  function standingsTable(context, rows = context.state.standings || [], { limit = null } = {}) {
    ...
    for (const row of limit ? rows.slice(0, limit) : rows) {
      ...
      tr.append(
        ...
        node('td', '', row.wins),
        node('td', '', row.losses),
        node('td', '', row.roundsFor),
        node('td', '', row.roundsAgainst),
        node(
          'td',
          row.roundDiff > 0 ? 'is-positive' : row.roundDiff < 0 ? 'is-negative' : '',
          row.roundDiff > 0 ? `+${row.roundDiff}` : row.roundDiff
        )
      );
      const stateCell = node('td');
      stateCell.append(
        row.tieRequiresAdmin
          ? badge('DESEMPATE', 'warning')
          : row.qualified === true
            ? badge('TOP 4', 'qualified')
            : row.qualified === false
              ? badge('FUERA', 'muted')
              : badge('EN JUEGO', 'pending')
      );
      tr.append(stateCell);
      body.append(tr);
    }
  ...
  function describe(context) {
    ...
    const descriptions = {
      hub: [
        ...
        event.name,
        'Todo el torneo, bien separado. Entra directamente en la fase, jornada o ranking que buscas.'
      ],
      regular: [
        'FASE REGULAR',
        'Todos contra todos',
        'Una lectura rápida de la liga antes de entrar en la tabla completa o en cada jornada.'
      ],
      standings: [
        'FASE REGULAR · CLASIFICACIÓN',
        'La tabla',
        `Victorias, desempates y seeding del 1º al 4º. ${playoffSpots(format, state.teams?.length || draft?.teams?.length || format?.teams || 0) === 'TODOS CLASIFICAN' ? 'Todos entran en playoffs.' : 'Sólo los cuatro primeros entran en playoffs.'}`
      ],
      matchdays: [
        'FASE REGULAR · JORNADAS',
        'El calendario',
        'Cada bloque cuenta una jornada. Abre cualquiera para ver sus cruces y mapas.'
      ],
      matchday: [
        `FASE REGULAR · JORNADA ${route.parameter || '—'}`,
        `Jornada ${route.parameter || '—'}`,
        'Todos los cruces de esta jornada en una única vista.'
      ],
      playoffs: [
        'ELIMINATORIAS',
        'El cuadro final',
        'Doble eliminación. El recorrido hacia la gran final, ronda a ronda.'
      ],
    ...
    const kpis =
      route.name === 'stats'
        ? [
            ...
            ['EQUIPOS', teamCount],
            ['MAPAS', state.seriesPlayed || 0]
          ]
        : route.name === 'playoffs'
          ? [
              ['ESTADO', playoffStatus(state)],
              ['EQUIPOS', format?.playoffs?.teams || teamCount],
              ['FORMATO', 'DOBLE ELIM.']
            ]
          : route.name === 'hub'
            ? [
                ['ESTADO', event.status],
                ['FORMATO', `${teamCount} × ${format?.teamSize || 5}`],
                ['LIGA', `${state.seriesPlayed || 0}/${state.seriesTotal || format?.regularSeason?.series || 0}`],
                ['PLAYOFFS', playoffSpots(format, teamCount)]
              ]
            : [
                ['ESTADO', event.status],
                ['EQUIPOS', teamCount],
                ['PARTIDOS', `${state.seriesPlayed || 0}/${state.seriesTotal || format?.regularSeason?.series || 0}`]
              ];
  ...
  function renderHub(context) {
    ...
    */
    const empezada = (context.state.teams?.length || 0) > 0 || (context.state.seriesTotal || 0) > 0;
    /*
    ...
    */
    ...
    if (next) {
      ...
      nextCard.append(
        node('h2', '', 'Todo al día'),
        node('p', 'section-copy', 'No hay ninguna serie pendiente con participantes confirmados.')
      );
    }
    spotlight.append(overview, nextCard);
    container.append(spotlight);

    const phases = node('section', 'hub-section');
    phases.append(
      sectionHeader(1, 'NAVEGACIÓN', 'Elige tu vista', 'Cada apartado tiene ahora su propia página y una sola función.')
    );
    ...
    [
      {
        ...
        title: 'Draft',
        copy: upcoming
          ? `${format?.captains || 4} capitanes · ${format?.draftPicks || 16} elecciones`
          : `${context.draft?.teams?.length || 0} equipos · ${statusLabel(context.draft?.status)}`,
        status: upcoming ? 'PRÓXIMAMENTE' : statusLabel(context.draft?.status),
        ready: context.draft?.status === 'COMPLETED',
        href: `${root}/draft`,
        accent: 'draft'
      },
      {
        ...
        title: 'Fase regular',
        copy: upcoming
          ? `${format?.regularSeason?.matchdays || 3} jornadas · ${format?.regularSeason?.series || 6} BO1`
          : `${context.state.seriesPlayed || 0} de ${context.state.seriesTotal || 0} partidos`,
        status: upcoming ? 'TODOS CONTRA TODOS' : context.state.complete ? 'TERMINADA' : 'EN JUEGO',
        ready: context.state.generated,
        href: `${root}/fase-regular`,
        accent: 'league'
      },
      {
        number: '03',
        title: 'Playoffs',
        copy: '1º–4º · 2º–3º · cuadro alto y bajo',
        status: upcoming ? 'DOBLE ELIMINACIÓN' : playoffStatus(context.state),
        ready: context.state.playoffs?.generated,
        href: `${root}/playoffs`,
        accent: 'playoffs'
      }
  ...
  function renderRegular(context) {
    ...
    if (context.state.preview) {
      ...
      section.append(
        sectionHeader(
          ...
          'FORMATO OFICIAL',
          'Todos contra todos',
          `${format.regularSeason.matchdays} jornadas · ${format.regularSeason.series} series · BO${format.regularSeason.bestOf}`
        )
      );
      section.append(
        node(
          'p',
          'competition-notice',
          format.teams > (format.playoffs?.teams || 4)
      ...
      for (let day = 1; day <= format.regularSeason.matchdays; day += 1) {
        ...
        card.append(
          node('span', 'matchday-index', String(day).padStart(2, '0')),
          node('h2', '', `Jornada ${day}`),
          node('p', 'section-copy', `${format.regularSeason.matchesPerMatchday} cruces · equipos por determinar`)
        );
        days.append(card);
      }
      section.append(days);
    ...
    if (!context.state.generated)
      return emptyState(
        'La liga todavía no está generada',
        'Cuando termine el draft, las jornadas y la tabla aparecerán aquí.',
        link(`${root}/draft`, 'primary-cta', 'VER DRAFT →')
      );
    ...
    progress.append(
      node('p', 'section-label', 'PROGRESO'),
      node('h2', '', context.state.complete ? 'Fase cerrada' : 'Liga en juego'),
      progressBar(context.state.seriesPlayed, context.state.seriesTotal, 'PARTIDOS DISPUTADOS')
    );
    summary.append(progress);
    layout.append(summary);
    const tableSection = node('section', 'content-section');
    tableSection.append(
      sectionHeader(2, 'CLASIFICACIÓN', 'Seeding de la liga', 'Todos avanzan; la posición decide los cruces.'),
      standingsTable(context, context.state.standings)
    );
    ...
    secondary.append(
      link(`${root}/estadisticas`, '', 'RANKING DE JUGADORES →'),
      link(`${root}/resultados`, '', 'TODOS LOS RESULTADOS →')
    );
  ...
  function renderStandings(context) {
    if (context.state.preview)
      return emptyState(
        ...
        'Los cuatro equipos se ordenarán del 1º al 4º. Todos entran en playoffs; las victorias y los desempates deportivos decidirán el seeding.'
      );
    if (!context.state.standings?.length)
      return emptyState('Sin clasificación', 'La tabla se calculará en cuanto exista la fase regular.');
    const section = node('section', 'content-section');
    section.append(
      sectionHeader(
        1,
        'TABLA COMPLETA',
        'Clasificación',
        'Las victorias mandan. Los desempates usan enfrentamiento directo y diferencia de rondas.'
      )
    );
    if (context.state.tieRequiresAdmin)
      section.append(
        node('p', 'competition-notice is-warning', 'Hay un empate que necesita resolución de la organización.')
      );
    section.append(standingsTable(context));
    const legend = node('div', 'table-legend');
    legend.append(
      badge('1º–4º', 'qualified'),
      node('span', '', 'Todos clasifican a playoffs'),
      badge('DESEMPATE', 'warning'),
      node('span', '', 'Orden pendiente de decisión')
    );
  ...
  function matchdaySummary(context, day) {
    ...
    const head = node('header');
    head.append(
      node('span', 'matchday-index', String(day.matchday).padStart(2, '0')),
      badge(
        completed === day.series.length ? 'FINALIZADA' : `${completed}/${day.series.length}`,
        completed === day.series.length ? 'done' : 'pending'
      )
    );
    ...
    for (const series of day.series) {
      const item = node('li');
      item.append(
        node('span', '', teamName(series.teamA)),
        node('b', '', scoreFor(series)),
        node('span', '', teamName(series.teamB))
      );
      matches.append(item);
    }
    card.append(matches, node('strong', 'phase-card-cta', 'ABRIR JORNADA →'));
    return card;
  }
  ...
  function renderMatchdays(context) {
    if (context.state.preview) {
      ...
      section.append(
        sectionHeader(
          ...
          'CALENDARIO OFICIAL',
          'Tres jornadas',
          'Dos enfrentamientos por jornada. Los equipos concretos se publicarán tras el draft.'
        )
      );
      const grid = node('div', 'matchday-grid');
      for (let day = 1; day <= 3; day += 1)
        grid.append(
          phaseCard(context, {
            ...
            title: `Jornada ${day}`,
            copy: 'Dos cruces · equipos por determinar',
            status: 'POR ANUNCIAR',
            href: '#',
            accent: 'calendar',
            cta: 'PENDIENTE'
    ...
    if (!context.state.matchdays?.length)
      return emptyState('Calendario pendiente', 'Las jornadas se publicarán al generar la fase regular.');
    const section = node('section', 'content-section');
    section.append(
      sectionHeader(1, 'CALENDARIO', 'Todas las jornadas', 'Un bloque por jornada. Entra para ver mapas y detalles.')
    );
    const grid = node('div', 'matchday-grid');
    context.state.matchdays.forEach((day) => grid.append(matchdaySummary(context, day)));
    section.append(grid);
  ...
  function renderMatchday(context) {
    ...
    if (!day)
      return emptyState(
        ...
        'No existe esa jornada en el calendario publicado.',
        link(`${competitionRoot(context.slug)}/fase-regular/jornadas`, 'primary-cta', 'VER TODAS →')
      );
    const section = node('section', 'content-section');
    ...
    section.append(
      sectionHeader(
        ...
        `${day.series.length} cruces · ${day.series.filter((series) => series.status === 'COMPLETED').length} finalizados`,
        back
      )
    );
    const grid = node('div', 'series-grid is-detail');
    day.series.forEach((series) =>
      grid.append(seriesCard(context, { ...series, stage: 'REGULAR', matchday: day.matchday }))
    );
    section.append(grid);
    return section;
  }
  ...
  function renderPlayoffs(context) {
    if (context.state.preview) {
      ...
      section.append(
        sectionHeader(
          1,
          'DOBLE ELIMINACIÓN',
          'Playoffs próximamente',
          'Los cuatro equipos entran. La primera derrota baja al cuadro inferior y la segunda elimina.'
        )
      );
      ...
      grid.append(
        phaseCard(context, {
          number: 'A',
          title: '1º vs 4º',
          copy: 'Primera semifinal del cuadro alto.',
          status: 'BO3',
          href: '#',
          accent: 'playoffs',
          cta: 'POR DETERMINAR'
        ...
        phaseCard(context, {
          number: 'B',
          title: '2º vs 3º',
          copy: 'Segunda semifinal del cuadro alto.',
          status: 'BO3',
          href: '#',
          accent: 'playoffs',
          cta: 'POR DETERMINAR'
      ...
      section.append(
        ...
        node(
          'p',
          'competition-notice',
          'La Gran Final será BO3 por defecto. Si el equipo del cuadro inferior vence al equipo invicto, se jugará una Gran Final de reset.'
        )
      );
      return section;
    }
    if (!context.state.playoffs?.generated)
      return emptyState(
        'El cuadro todavía no está generado',
        context.state.complete
          ? context.event.officialFormat
            ? 'La liga ya ha terminado. La organización publicará los cruces 1º–4º y 2º–3º.'
            : 'La liga ya ha terminado. La organización publicará los cruces del Top 4.'
          : 'Primero deben terminar todos los partidos de la fase regular.',
        link(`${competitionRoot(context.slug)}/fase-regular/clasificacion`, 'primary-cta', 'VER CLASIFICACIÓN →')
      );
    const section = node('section', 'content-section bracket-section');
    section.append(
      sectionHeader(
        ...
        'DOBLE ELIMINACIÓN',
        'Camino a la final',
        'Dos derrotas eliminan. El cuadro alto y el bajo convergen en la gran final.'
      )
    );
    ...
    for (const [key, title] of zones) {
      ...
      context.state.playoffs.series
        .filter((series) => series.bracket === key)
        .forEach((series) => zone.append(seriesCard(context, { ...series, stage: 'PLAYOFFS' })));
      bracket.append(zone);
    }
    ...
    if (confirmedPlacements.length) {
      ...
      confirmedPlacements.forEach((place) => {
        const item = node('li');
        item.append(node('b', '', `#${place.position}`), node('span', '', place.name || `Equipo ${place.teamId}`));
        list.append(item);
      });
      placements.append(list);
      section.append(placements);
    }
    const secondary = node('nav', 'competition-secondary-links');
  ...
  function playerNameMap(context) {
    return new Map(
      (context.state.teams || []).flatMap((team) =>
  ...
  function renderStatsTable(context, rows, metric) {
    ...
    const head = node('thead');
    const header = node('tr');
    ['#', 'JUGADOR', 'EQUIPO', 'PJ', 'GLOBAL', 'ACS', 'K', 'D', 'A', 'K/D', 'ADR', 'HS%', 'KAST', 'FB'].forEach(
      (label) => {
        const cell = node('th', '', label);
        cell.scope = 'col';
        header.append(cell);
    ...
    rows.forEach((row, index) => {
      ...
      const position = node('th', 'position-cell', String(index + 1).padStart(2, '0'));
      position.scope = 'row';
      const value = (field) => (row[field] === null || row[field] === undefined ? '—' : row[field]);
      tr.append(
        position,
        node('td', 'team-name-cell', names.get(row.participantId) || `Jugador ${row.participantId}`),
        node('td', '', teams.get(row.teamId) || '—'),
        node('td', '', value('games')),
        node('td', metric === 'overall' ? 'is-highlight global-score-cell' : 'global-score-cell', value('globalScore')),
        node('td', metric === 'acs' ? 'is-highlight' : '', value('acs')),
        node('td', metric === 'kills' ? 'is-highlight' : '', value('kills')),
        node('td', '', value('deaths')),
        node('td', metric === 'assists' ? 'is-highlight' : '', value('assists')),
        node('td', metric === 'kd' ? 'is-highlight' : '', value('kd')),
        node('td', '', value('adr')),
        node('td', '', value('hsPercent')),
        node('td', '', value('kastPercent')),
        node('td', metric === 'firstKills' ? 'is-highlight' : '', value('firstKills'))
      );
      body.append(tr);
    });
    table.append(head, body);
  ...
  function renderStats(context) {
    ...
    if (!rawStats.length)
      return emptyState(
        ...
        context.state.preview
          ? 'Las estadísticas aparecerán cuando comiencen los partidos.'
          : 'Aparecerán cuando existan partidas con datos de jugadores confirmados.'
      );
    ...
    section.append(
      sectionHeader(
        ...
        'Una lectura conjunta del rendimiento confirmado durante todo el torneo.'
      )
    );
    const formula = node('aside', 'global-ranking-note');
    formula.append(
      node('strong', '', 'ÍNDICE GLOBAL · 0–100'),
      node(
        'p',
        '',
        'Combina todas las fases y ACS, K/D, ADR, KAST, kills, deaths, asistencias, headshots y primeras bajas y muertes. Compara promedios por partida, pondera la muestra real y los datos ausentes no suman.'
      )
    );
    section.append(formula);
    const controls = node('form', 'stats-controls');
    controls.addEventListener('submit', (event) => event.preventDefault());
    const searchLabel = node('label');
    searchLabel.append(node('span', '', 'BUSCAR JUGADOR'));
    const search = node('input');
    search.type = 'search';
    search.name = 'q';
    search.autocomplete = 'off';
    search.placeholder = 'Nombre del jugador…';
    searchLabel.append(search);
    const metricLabel = node('label');
    metricLabel.append(node('span', '', 'ORDENAR POR'));
    const metric = node('select');
    metric.name = 'metric';
    ...
    teamLabel.append(node('span', '', 'EQUIPO'));
    const team = node('select');
    team.name = 'team';
    const all = node('option', '', 'Todos los equipos');
    all.value = '';
    team.append(all);
    ...
    controls.append(searchLabel, metricLabel, teamLabel);
    ...
    const repaint = () => {
      ...
      const rows = View.rankPlayers(
        stats.filter(
          (row) =>
            (!team.value || Number(team.value) === row.teamId) &&
            (!normalizedSearch ||
              (names.get(row.participantId) || '').toLocaleLowerCase('es').includes(normalizedSearch))
        ),
        metricField,
        metric.value === 'deaths' ? 'asc' : 'desc'
      );
      /*
    ...
    [search, metric, team].forEach((control) =>
      control.addEventListener(control === search ? 'input' : 'change', update)
    );
    repaint();
    section.append(podium, tableTarget);
    ...
    teamSection.append(
      sectionHeader(
        ...
        'Datos por equipo',
        'El récord de la fase regular contextualiza los rankings individuales.'
      )
    );
    teamSection.append(recordCards(context));
    const stack = node('div', 'page-stack');
    stack.append(section, teamSection);
    return stack;
  }
  ...
  function renderResults(context) {
    ...
    if (!completed.length)
      return emptyState(
        ...
        context.state.preview
          ? 'Los marcadores aparecerán cuando comiencen los partidos.'
          : 'Los partidos cerrados aparecerán aquí, separados del calendario pendiente.'
      );
    ...
    section.append(
      sectionHeader(
        ...
        'Filtra por fase o equipo y abre cualquier serie para ver sus mapas.'
      )
    );
    const controls = node('form', 'stats-controls results-controls');
    controls.addEventListener('submit', (event) => event.preventDefault());
    const stageLabel = node('label');
    stageLabel.append(node('span', '', 'FASE'));
    ...
    if ([...team.options].some((option) => option.value === query.get('team'))) team.value = query.get('team');
    controls.append(stageLabel, teamLabel);
    section.append(controls);
    const target = node('div');
    const repaint = () => {
      const rows = completed.filter(
        (series) =>
          (!stage.value || series.stage === stage.value) &&
          (!team.value || [series.teamA?.teamId, series.teamB?.teamId].includes(Number(team.value)))
      );
      const grid = node('div', 'result-list');
      rows.reverse().forEach((series) => grid.append(seriesCard(context, series, { compact: true })));
    ...
    [stage, team].forEach((control) => control.addEventListener('change', update));
    repaint();
    section.append(target);
    const privacy = node('aside', 'privacy-note');
    privacy.append(
      node('strong', '', 'CAPTURAS PROTEGIDAS'),
      node(
        'p',
        '',
        'Las capturas originales pueden contener información privada y sólo se consultan en administración. La parte pública muestra únicamente el resultado y las estadísticas confirmadas.'
      )
    );
  ...
  function renderMatch(context) {
    ...
    if (!series)
      return emptyState(
        ...
        'La serie no existe o ya no forma parte de esta competición.',
        link(`${competitionRoot(context.slug)}/resultados`, 'primary-cta', 'VOLVER A RESULTADOS →')
      );
    const section = node('section', 'content-section match-detail');
    const stage =
      series.stage === 'PLAYOFFS' ? series.label || 'Playoffs' : `Fase regular · Jornada ${series.matchday}`;
    ...
    const side = (team, winner) => {
      const box = node('div', winner ? 'is-winner' : '');
      box.append(node('span', '', winner ? 'GANADOR' : 'EQUIPO'), node('h2', '', teamName(team)));
      return box;
    };
    teams.append(
      side(series.teamA, series.winnerTeamId === series.teamA?.teamId),
      node('strong', '', scoreFor(series)),
      side(series.teamB, series.winnerTeamId === series.teamB?.teamId)
    );
    hero.append(
      teams,
      node(
        'p',
        'match-date',
        series.scheduledAt ? DATE_TIME.format(new Date(series.scheduledAt)) : 'Fecha y hora por confirmar'
      )
    );
    section.append(hero);
    const games = node('section', 'match-games');
    games.append(sectionHeader(1, 'MAPAS', 'Desglose de la serie', 'Sólo aparecen resultados confirmados.'));
    const grid = node('div', 'series-grid is-detail');
    (series.games || [])
      .filter((game) => game.status !== 'NOT_NEEDED')
      .forEach((game) => {
        const card = node('article', 'map-detail-card');
        card.append(
          node('span', 'section-label', `MAPA ${game.gameNumber}`),
          node('h3', '', mapName(context, game.mapKey)),
          node(
            'strong',
            'map-score',
            game.status === 'COMPLETED' ? `${game.teamARounds} — ${game.teamBRounds}` : statusLabel(game.status)
        ...
        if (game.stats?.length && context.openStats) {
          const button = node('button', 'primary-cta', 'VER ESTADÍSTICAS');
          button.type = 'button';
          button.addEventListener('click', () => context.openStats(series, game));
          card.append(button);
        }
    ...
    section.append(
      node(
        'p',
        'privacy-note',
        'Las imágenes originales de las capturas se mantienen privadas. Aquí sólo se publica la información deportiva confirmada.'
      )
    );
    return section;
  }
  ...
  return {
    ...
    describe,
    render(context) {
      return (renderers[context.route.name] || renderHub)(context);
    },
    statusLabel,
    mapName,
    node
