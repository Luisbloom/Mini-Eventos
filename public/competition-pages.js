...
(() => {
  ...
  async function loadData() {
    ...
    if (eventBody.event.status === 'Próximamente') {
      return {
        ...
        state: View.previewCompetitionState(eventBody.event.officialFormat),
        draft: null
      };
    }
  ...
  function buildNavigation() {
    ...
    nav.replaceChildren(
      ...View.navItems(route.slug).map((item) => {
  ...
  function drawStatsDialog(context, series, game) {
    ...
    byId('dialog-score').textContent =
      `${game.teamARounds} — ${game.teamBRounds} · ${Renderers.mapName(context, game.mapKey)}`;

    const table = byId('dialog-table');
    const head = document.createElement('thead');
    const header = document.createElement('tr');
    ['JUGADOR', 'EQUIPO', 'AGENTE', 'ACS', 'K', 'D', 'A', '+/−', 'ADR', 'HS%', 'KAST', 'FB'].forEach((label) => {
    ...
    [...(game.stats || [])]
      .sort((left, right) => left.teamId - right.teamId || (right.acs || 0) - (left.acs || 0))
      .forEach((row) => {
        const tr = document.createElement('tr');
        [
          playerNames.get(row.participantId) || `Jugador ${row.participantId}`,
  ...
  function draw({ event, state, draft }) {
    ...
    byId('competition-kpis').replaceChildren(
      ...description.kpis.map(([label, value]) => {
    ...
    connection(
      state.preview ? 'loading' : 'live',
      state.preview
        ? 'PRÓXIMAMENTE'
        : state.playoffs?.status === 'IN_PROGRESS' || !state.complete
          ? 'EN DIRECTO'
          : 'ACTUALIZADO'
    );
  }
  ...
  async function refresh() {
    ...
    try {
      ...
      showError(
        error.code === 'EVENT_NOT_FOUND' ? 'Competición no encontrada' : 'No se pudo cargar',
  ...
  function connectStream() {
    ...
    [
      ...
      'pick_made',
      'team_updated'
    ].forEach((event) => stream.addEventListener(event, refresh));
