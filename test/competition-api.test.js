...
describe('competition API', () => {
  ...
  const adminToken = 'admin-test',
    reporterToken = 'reporter-test';
  const admin = (method, url) => request(app)[method](url).set('Authorization', `Bearer ${adminToken}`);
  beforeEach(() => {
    directory = fs.mkdtempSync(path.join(os.tmpdir(), 'jartiland-competition-api-'));
    ...
    stage = database.competition.listStages(event.id)[0];
    database.competition.updateStage(stage.id, { status: 'active' });
    stage = database.competition.getStage(stage.id);
    group = database.competition.listGroups(stage.id)[0];
    host = database.competition.listHosts(event.id)[0];
    participant = database.createParticipant(event.id, {
      discord_username: 'privado',
      game_name: 'Jugador',
      friend_code: 'SECRET#1'
    });
    database.updateParticipant(participant.id, { status: 'confirmed', internalFriendCode: 'SECRET#1' });
    database.competition.distributeGroups(stage.id);
    group = database.competition
      .listGroups(stage.id)
      .find((item) => database.competition.listStageParticipants(stage.id, item.id).length);
  });
  afterEach(() => {
    database.close();
  ...
  function structured(reportId = 'structured-1') {
    return {
      reportId,
      eventId: event.id,
      stageId: stage.id,
      ...
      matchNumber: 1,
      playedAt: '2026-08-21T16:10:00+02:00',
      map: 'The Skeld',
      winnerTeam: 'crew',
      players: [{ participantId: participant.id, role: 'Crewmate', won: true, tasksCompleted: 4, tasksTotal: 4 }]
    };
  }
  ...
  it('publishes phases, schedule and prizes without private participant data', async () => {
    ...
    await request(app)
      .get(`/api/events/${event.slug}/schedule`)
      .expect(200)
      .expect((response) => assert.equal(response.body.schedule.length, 5));
    await request(app)
      .get(`/api/events/${event.slug}/prizes`)
      .expect(200)
      .expect((response) => assert.equal(response.body.prizes.length, 4));
  });
  ...
  it('validates Reporter scope, keeps reportId idempotent and never exposes the raw payload', async () => {
    ...
    const other = database.createEvent({
      name: 'Otro',
      slug: 'otro',
      game: 'Otro',
      description: '',
      modules: { matches: true, competition: true }
    });
  ...
  it('isolates simultaneous hosts and groups while preserving idempotency and one VALID result per slot', async () => {
    const secondParticipant = database.createParticipant(event.id, {
      discord_username: 'paralelo',
      game_name: 'Jugador paralelo',
      friend_code: 'paralelo#2'
    });
    ...
    const first = await Promise.all(
      reports.map((report, index) =>
        request(app)
          .post('/api/matches')
          .set('Authorization', `Bearer ${index === 0 ? token1 : token2}`)
          .send(report)
      )
    );
    ...
    assert.deepEqual(
      replay.map((response) => response.body.id),
      first.map((response) => response.body.id)
    );
    assert.equal(database.countMatches(event.id), 2);
    ...
    const collisionResponses = await Promise.all(
      collisions.map((report) =>
        request(app).post('/api/matches').set('Authorization', `Bearer ${token1}`).send(report)
      )
    );
  ...
  it('accepts only exact authenticated replays after a stage changes state', async () => {
    ...
    const created = await request(app)
      ...
      .send(original)
      .expect(201);
    const equivalentInstant = await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${token1}`)
      .send({ ...original, playedAt: '2026-08-21T14:10:00.000Z' })
      .expect(200);
    assert.equal(equivalentInstant.body.id, created.body.id);
    const missingPlayedAt = { ...original };
    delete missingPlayedAt.playedAt;
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${token1}`)
    ...
    const replayAfterParticipantChange = await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${token1}`)
      .send({ ...original })
      .expect(200);
  ...
  it('does not attach audit names from competitive ids belonging to another event', async () => {
    ...
    const foreignStage = database.competition.createStage(other.id, {
      name: 'Fase ajena',
      type: 'group_stage',
      position: 1,
      matchesPerGroup: 1
    ...
    const foreignHost = database.competition.replaceHosts(other.id, [
      { name: 'Host ajeno', identifier: 'FOREIGN_HOST', enabled: true }
    ])[0];
  ...
  it('authenticates each Reporter host independently and updates only successful activity', async () => {
    ...
    const other = database.createEvent({
      ...
      game: 'Otro',
      description: '',
      modules: { matches: true }
    });
    await request(app)
      .post(`/api/events/${other.slug}/matches`)
      .set('Authorization', `Bearer ${token2}`)
  ...
  it('creates one-time Reporter configuration and revokes it without exposing hashes', async () => {
    ...
    assert.equal(
      database.competition.findHostByReporterTokenHash(event.id, hashReporterToken(previousToken)).id,
      host.id
    );
  ...
  it('uses the same ingestion for simulator, exposes scoring breakdown privately and supports VOID', async () => {
    const simulated = await admin('post', `/api/admin/events/${event.id}/simulator`)
      .send({ report: structured('simulated-1') })
      .expect(201);
    ...
    await admin('patch', `/api/admin/events/${event.id}/matches/${simulated.body.match.id}/void`)
      .send({ reason: 'Desconexión general' })
      .expect(200);
  ...
  it('rejects inactive phases, invalid or occupied slots and duplicate players', async () => {
    ...
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${reporterToken}`)
      .send({ ...structured('pending-stage'), stageId: pending.id, groupId: null })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'STAGE_NOT_ACTIVE'));
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${reporterToken}`)
      .send({ ...structured('too-many'), matchNumber: stage.matchesPerGroup + 1 })
      .expect(400)
      .expect((response) => assert.equal(response.body.error.code, 'MATCH_NUMBER_OUT_OF_RANGE'));
    const duplicated = { ...structured('duplicate-player'), matchNumber: 2 };
    duplicated.players = [duplicated.players[0], { ...duplicated.players[0] }];
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${reporterToken}`)
      .send(duplicated)
      .expect(400)
      .expect((response) => assert.equal(response.body.error.code, 'DUPLICATE_REPORT_PLAYER'));
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${reporterToken}`)
      .send({ ...structured('slot-first'), matchNumber: 3 })
      .expect(201);
    await request(app)
      .post('/api/matches')
      .set('Authorization', `Bearer ${reporterToken}`)
      .send({ ...structured('slot-second'), matchNumber: 3 })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'MATCH_SLOT_OCCUPIED'));
  });

  it('cannot bypass the audited completion workflow through the generic stage editor', async () => {
    await admin('put', `/api/admin/stages/${stage.id}`)
      .send({ status: 'completed' })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'USE_STAGE_COMPLETION'));
    assert.equal(database.competition.getStage(stage.id).status, 'active');
    const pending = database.competition.listStages(event.id)[1];
    await admin('post', `/api/admin/stages/${pending.id}/complete`)
      .send({ force: true })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'STAGE_NOT_ACTIVE'));
    ...
    await admin('put', `/api/admin/stages/${stage.id}`)
      .send({ status: 'active' })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'STAGE_ALREADY_COMPLETED'));
  });

  it('requires a group for group-stage leaderboards and respects the competition module', async () => {
    await request(app)
      .get(`/api/events/${event.slug}/stages/${stage.id}/leaderboard`)
      .expect(400)
      .expect((response) => assert.equal(response.body.error.code, 'GROUP_REQUIRED'));
    database.updateEvent(event.id, { modules: { ...event.modules, competition: false } });
    await request(app)
      .get(`/api/events/${event.slug}/stages/${stage.id}/leaderboard?groupId=${group.id}`)
      .expect(404)
      .expect((response) => assert.equal(response.body.error.code, 'MODULE_DISABLED'));
  });

  it('records legacy admin results as MANUAL with an admin actor', async () => {
    const created = await admin('post', `/api/admin/events/${event.id}/matches`)
      .send({ report: { reportId: 'manual-legacy', players: [] } })
      .expect(201);
    ...
    await admin('post', `/api/admin/events/${event.id}/matches`)
      .send({
        report: { reportId: 'manual-partial-context', players: [] },
        context: { groupId: group.id, matchNumber: 5 }
      })
      .expect(400)
      .expect((response) => assert.equal(response.body.error.code, 'STAGE_REQUIRED'));
    const scoped = structured('manual-context');
    const context = {
      stageId: scoped.stageId,
      groupId: scoped.groupId,
      hostId: scoped.hostId,
      matchNumber: 4,
      playedAt: scoped.playedAt
    };
    delete scoped.eventId;
    delete scoped.stageId;
    delete scoped.groupId;
    delete scoped.hostId;
    delete scoped.matchNumber;
    delete scoped.playedAt;
    const competitive = await admin('post', `/api/admin/events/${event.id}/matches`)
      .send({ report: scoped, context })
      .expect(201);
    assert.equal(competitive.body.origin, 'MANUAL');
    assert.equal(competitive.body.stageId, stage.id);
    assert.equal(competitive.body.groupId, group.id);
    assert.equal(competitive.body.matchNumber, 4);
    const equivalentContext = { ...context, playedAt: '2026-08-21T14:10:00.000Z' };
    const manualReplay = await admin('post', `/api/admin/events/${event.id}/matches`)
      .send({ report: scoped, context: equivalentContext })
      .expect(200);
    assert.equal(manualReplay.body.id, competitive.body.id);
    const contextWithoutPlayedAt = { ...context };
    delete contextWithoutPlayedAt.playedAt;
    await admin('post', `/api/admin/events/${event.id}/matches`)
      .send({ report: scoped, context: contextWithoutPlayedAt })
      .expect(409)
      .expect((response) => assert.equal(response.body.error.code, 'REPORT_ID_CONFLICT'));
  });
});
