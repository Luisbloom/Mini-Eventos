...
describe('competition domain', () => {
  it('balances confirmed participant ids with a maximum difference of one', () => {
    const twenty = balanceParticipants(
      Array.from({ length: 20 }, (_, index) => index + 1),
      [10, 20]
    );
    assert.deepEqual(
      twenty.map((group) => group.participantIds.length),
      [10, 10]
    );
    const twentyOne = balanceParticipants(
      Array.from({ length: 21 }, (_, index) => index + 1),
      [10, 20]
    );
    assert.deepEqual(
      twentyOne.map((group) => group.participantIds.length),
      [11, 10]
    );
  });
  ...
  it('orders ties by points, wins, impostor wins, full tasks and kills', () => {
    ...
    fields.forEach((field) => {
      const first = { points: 8, wins: 2, impostorWins: 1, allTasksGames: 1, kills: 1, [field]: 2 };
      const second = { points: 8, wins: 2, impostorWins: 1, allTasksGames: 1, kills: 1, [field]: 1 };
      for (const previous of fields.slice(0, fields.indexOf(field))) first[previous] = second[previous];
      assert.equal(automaticCompare(first, second) < 0, true);
    });
  });
  ...
  it('shares absolute positions and flags an unresolved cutoff tie', () => {
    ...
    assert.deepEqual(
      result.standings.map((row) => row.rank),
      [1, 1, 3]
    );
  ...
  it('applies an audited resolution only after all automatic criteria tie', () => {
    ...
    assert.deepEqual(
      result.standings.map((row) => row.participantId),
      [2, 1]
    );
  ...
  it('requires a complete auditable order when three players tie at the cutoff', () => {
    const matches = [
      match([
        { participantId: 1, name: 'Ana', role: 'Crewmate', won: true },
        { participantId: 2, name: 'Bea', role: 'Crewmate', won: true },
        { participantId: 3, name: 'Cris', role: 'Crewmate', won: true }
      ])
    ];
    const partial = buildCompetitionLeaderboard(matches, {
      qualifiers: 1,
      resolutions: [{ higherParticipantId: 2, lowerParticipantId: 1 }]
    });
    assert.equal(partial.cutoffTie, true);
    assert.deepEqual(new Set(partial.decisiveTieParticipantIds), new Set([1, 2, 3]));
    assert.deepEqual(
      partial.standings.map((row) => row.rank),
      [1, 1, 1]
    );
    const complete = buildCompetitionLeaderboard(matches, {
      qualifiers: 1,
      resolutions: [
        { higherParticipantId: 2, lowerParticipantId: 1 },
        { higherParticipantId: 1, lowerParticipantId: 3 }
      ]
    });
    assert.equal(complete.cutoffTie, false);
    assert.deepEqual(
      complete.standings.map((row) => row.participantId),
      [2, 1, 3]
    );
    assert.deepEqual(
      complete.standings.map((row) => row.rank),
      [1, 2, 3]
    );
  });
  ...
  it('ignores points declared by the Reporter and always applies canonical scoring', () => {
    const result = buildCompetitionLeaderboard([
      match([{ participantId: 1, name: 'Intento', role: 'Crewmate', won: false, points: 999, score: 999 }])
    ]);
