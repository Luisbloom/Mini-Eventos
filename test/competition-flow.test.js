...
describe('complete tournament flow',()=>{
  ...
  it('runs 20 players through 10/10 groups, top five, reset final and champion',()=>{
    ...
    for(let index=1;index<=20;index++){const participant=database.createParticipant(event.id,{discord_username:`qa${index}`,game_name:`Jugador ${String(index).padStart(2,'0')}`,friend_code:`qa#${index}`});database.updateParticipant(participant.id,{status:'confirmed',internalFriendCode:`QA#${index}`});}
  ...
  it('refuses a duplicate-slot migration clearly without rewriting existing results',()=>{
    ...
    assert.throws(
      ()=>openDatabase(dbPath),
      (error)=>error.code==='MATCH_SLOT_DUPLICATES_EXIST'&&error.status===500&&/Anula los duplicados/.test(error.message)
    );
    ...
    raw.prepare("UPDATE matches SET match_status='VOID',void_reason='Duplicado histórico revisado' WHERE id=(SELECT MAX(id) FROM matches)").run();
    raw.close();
