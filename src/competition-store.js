...
function createCompetitionStore(connection) {
  ...
  return {
    ...
    resolveReportPlayerIdentities(eventId,players){if(!Array.isArray(players))throw new CompetitionError('El resultado necesita jugadores.','REPORT_PLAYERS_REQUIRED');return players.map((player)=>{let participantId=Number(player.participantId);let participant=Number.isInteger(participantId)&&participantId>0?connection.prepare('SELECT * FROM event_participants WHERE id=? AND event_id=?').get(participantId,eventId):null;if(!participant&&player.friendCode)participant=connection.prepare('SELECT * FROM event_participants WHERE event_id=? AND internal_friend_code=?').get(eventId,normalizeFriendCode(player.friendCode));if(!participant)throw new CompetitionError('No se ha podido vincular un jugador con su inscripción.','PLAYER_NOT_LINKED',409);const copy={...player,participantId:participant.id,name:participant.display_name};delete copy.friendCode;delete copy.points;delete copy.score;return copy;});},
    resolveReportPlayers(eventId,stageId,groupId,players){if(!Array.isArray(players)||players.length<1)throw new CompetitionError('El resultado necesita jugadores.','REPORT_PLAYERS_REQUIRED');return players.map((player)=>{let participantId=Number(player.participantId);let participant=Number.isInteger(participantId)&&participantId>0?connection.prepare('SELECT * FROM event_participants WHERE id=? AND event_id=?').get(participantId,eventId):null;if(!participant&&player.friendCode)participant=connection.prepare('SELECT * FROM event_participants WHERE event_id=? AND internal_friend_code=?').get(eventId,normalizeFriendCode(player.friendCode));if(!participant)throw new CompetitionError('No se ha podido vincular un jugador con su inscripción.','PLAYER_NOT_LINKED',409);const membership=connection.prepare('SELECT * FROM stage_participants WHERE stage_id=? AND participant_id=?').get(stageId,participant.id);if(participant.status==='disqualified'||membership?.competitive_status==='disqualified')throw new CompetitionError(`${participant.display_name} está descalificado.`,'PLAYER_DISQUALIFIED',409);if(participant.status!=='confirmed'||!membership||(groupId!==null&&membership.group_id!==groupId))throw new CompetitionError(`${participant.display_name} no pertenece a esta fase o grupo.`,'PLAYER_SCOPE_MISMATCH',409);const copy={...player,participantId:participant.id,name:participant.display_name};delete copy.friendCode;delete copy.points;delete copy.score;return copy;});}
  };
}
